...
from dagster.core.execution.plan.step import DynamicStepHandle, StepHandle
...
def _update_tracking_dict(tracking, step_key):
    handle = StepHandle.from_key(step_key)
    if isinstance(handle, DynamicStepHandle):
...
def get_retry_steps_from_execution_plan(instance, execution_plan, parent_run_id):
    ...
    for step in execution_plan.topological_steps():
        ...
        if retrying_deps:
            step_handle = StepHandle.from_key(step.key)
            for retrying_key in retrying_deps:
                retrying_handle = StepHandle.from_key(retrying_key)
                if isinstance(retrying_handle, UnresolvedStepHandle) and isinstance(
                    ...
                    for resolved_key in resolved_keys:
                        resolved_handle = StepHandle.from_key(resolved_key)
                        check.invariant(
                            isinstance(resolved_handle, DynamicStepHandle),
                            "Expected DynamicStepHandle",
                        )
